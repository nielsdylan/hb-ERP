            ...
            <div class="col-md-4">
                <div class="card">
                    ...
                    <div class="card-body d-flex flex-column">
                        <h3>
                            <a href="{{ route('hb.mis-cursos.curso', ['codigo'=>$value->aula->codigo]) }}">{{$value->curso->nombre}}</a>
                        </h3>
                        <div class="text-muted">
                            {{$value->codigo}}
                            <small class="d-block text-muted"> Fecha: {{ date("d/m/Y", strtotime($value->fecha)) }}</small>
                            <small class="d-block text-muted">H. Inicio: {{$value->aula->hora_inicio }}</small>
                            <small class="d-block text-muted">H. Final: {{$value->aula->hora_final }}</small>
                        </div>
                        <div class="d-flex align-items-center pt-5 mt-auto">
                            <span class="avatar avatar-md brround bg-success me-3">{{$value->aula->usuario->avatar_initials}}</span>
                            <div>
                                <small class="d-block text-muted"> Docente:</small>
                                <a href="javascript:void(0)" class="text-default">{{$value->aula->usuario->nombre_corto}}</a>
                            </div>
                            <div class="ms-auto">
